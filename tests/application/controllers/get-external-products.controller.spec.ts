...
import { GetExternalProductsController } from "../../../src/application/controllers/get-external-products.controller";
...
describe('Controllers: GetExternalProductsController', () => {
  let sut: GetExternalProductsController
  ...
  beforeEach(() => {
    sut = new GetExternalProductsController(useCase)
  })

  it('should call use case with correct input', async () => {
    await sut.execute({ page: 1, perPage: 10 })

    expect(useCase.execute).toHaveBeenCalledWith({ page: 1, perPage: 10, external: true })
